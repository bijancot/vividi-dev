...
function wppcp_version2_upgrade_notice() {

    if( current_user_can('manage_options') || current_user_can('wppcp_manage_options') ){

        if ( isset($_POST['wppcp_upgrade_notice_nonce_field']) ){

            if( ! wp_verify_nonce( $_POST['wppcp_upgrade_notice_nonce_field'], 'wppcp_upgrade_notice_nonce' ) ) {
                return;
            }else{
                update_option('wppcp_version2_upgrade_notice','read');
            }
        }

        if ( get_option('wppcp_version2_upgrade_notice') == '' ) {    
    ?>                
        <div style="background: #ffbaba;font-weight: bold;" class="notice notice-error is-dismissible">
            <p><?php _e('WP Private Content Plus Version 2.1 resolves a major security issue in settings section. If the site is already hacked, please click ','wppcp');?>
                <a href='https://www.wpexpertdeveloper.com/major-security-issue-in-wp-private-content-plus-versions-1-31/'><?php _e('HERE','wppcp'); ?></a><?php _e(' to find more details and resolve the issues.','wppcp');?>
            <form method="POST">
                <?php wp_nonce_field( 'wppcp_upgrade_notice_nonce', 'wppcp_upgrade_notice_nonce_field' );  ?> 
                <?php $value = __('Remove Notice','wppcp') ; 
                        submit_button($value); ?>
            </form>
            </p>
        </div>
...
add_action('admin_notices', 'wppcp_version2_upgrade_notice');
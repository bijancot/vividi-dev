...
class Pagseguro extends \Indeed\Ihc\PaymentGateways\PaymentAbstract
{
    ...
    public function doPayment()
    {
		...
		if($intervalType == "Monthly"){
          ...
          //trial block
          if ( \Ihc_Db::level_has_trial_period( $this->attributes['lid'] ) ){
            //isset($levelData['access_regular_time_value']) && $levelData['access_regular_time_value']!==''){
            /// TAXES
          ...
          $requestData = array(
                                  ...
                                  //'preApprovalCode'                         => md5( $thia->attributes['uid'] . $thia->attributes['lid'] . $thia->attributes['orderId'] ),
          );

          if ( ( !empty($trial) || !empty($couponData) ) && !empty($levelData['access_trial_price'])){

...
class WPMLActions
{
    ...
    {
        /*
        if ( indeed_is_plugin_active( 'wpml-translation-management/plugin.php' ) ){
            return false;
        }
        */
    ...
    {
        global $wpdb;
        $data = $wpdb->get_results( "SELECT notification_type, level_id, subject, message, pushover_message FROM {$wpdb->prefix}ihc_notifications;" );
        if ( !$data ){
            return;
        }
        $domain = 'ihc';
    ...
    {
        global $wpdb;
        $data = $wpdb->get_results( "SELECT id, label, description FROM {$wpdb->prefix}ihc_taxes;" );
        if ( !$data ){
            return;
        }
        $domain = 'ihc';

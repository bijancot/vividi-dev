				...
				<div class="container">
					<div id="main">
						...
						<p class="light-blue-color block" style="font-size: 1.3333em;">
							...
							} else if ( isset( $_GET['action'] ) && ( $_GET['action'] == 'Lupa Password' ) ) {
								echo __( 'Please enter your username or email address.', 'trav') . '<br />' . __( 'Kamu akan menerima konfirmasi reset password melalui email.', 'trav' );
							} else if ( isset( $_GET['checkemail'] ) ) {
								_e( 'Cek email kamu untuk konfirmasi pendaftaran dan reset password.', 'trav' );
							} else {
								_e( 'Mohon Login dahulu.', 'trav' );
							}
							?>
						</p>
						...
						<div class="col-sm-8 col-md-6 col-lg-5 no-float no-padding center-block">
							...
							<?php if ( isset( $_GET['action'] ) && ( $_GET['action'] == 'register' ) ) { ?>
								...
								<form name="registerform" class="login-form" action="<?php echo site_url('wp-login.php?action=register', 'login_post'); ?>" method="post" novalidate="novalidate">
							...
							<?php } else if ( isset( $_GET['action'] ) && ( $_GET['action'] == 'lostpassword' ) ) { ?>
								...
								<form name="lostpasswordform"  class="login-form" action="<?php echo esc_url( wp_lostpassword_url() ) ?>" method="post">
									...
									<p><br />
										<a href="<?php echo esc_url( $login_url );?>" class="underline"><?php _e( 'Login', 'trav' ); ?></a> | 
										<a href="<?php echo esc_url( $signup_url ); ?>" class="underline"><?php _e( "Daftar", 'trav' ) ?></a>
									</p>
								</form>

...
<div class="view-profile">
		...
		<div class="details">
			...
			<dl class="term-description">
				...
				if ( ! empty( $user_info['first_name'] ) ) { 
					?>
					<dt><?php echo __( 'Nama Depan', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['first_name'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['last_name'] ) ) { 
					?>
					<dt><?php echo __( 'Nama Belakang', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['last_name'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['phone'] ) ) { 
					?>
					<dt><?php echo __( 'Telepon', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['phone'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['birthday'] ) ) { 
					?>
					<dt><?php echo __( 'Tanggal Lahir', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['birthday'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['address'] ) ) { 
					?>
					<dt><?php echo __( 'Alamat Lengkap', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['address'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['city'] ) ) { 
					?>
					<dt><?php echo __( 'Kota', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['city'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['zip'] ) ) { 
					?>
					<dt><?php echo __( 'Kode Pos', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['zip'] ) ?></dd>
					<?php 
				} 
				?>
				...
				if ( ! empty( $user_info['country'] ) ) { 
					?>
					<dt><?php echo __( 'Negara', 'trav' ) ?>:</dt><dd><?php echo esc_html( $user_info['country'] ) ?></dd>
					<?php 
				} 
				?>
			</dl>
	...
	<hr>
	<h2><?php echo __( 'Tentang Kamu !', 'trav' ) ?></h2>
		<div class="intro">
		<p><?php echo esc_html( $user_info['description'] ); ?></p>
	</div>
</div>
<div class="edit-profile">
	<a href="#" class="button btn-mini pull-right view-profile-btn"><?php echo __( 'LIHAT PROFIL KAMU', 'trav' ) ?></a>
	<form class="edit-profile-form" method="post" enctype='multipart/form-data'>
		...
		<div class="col-sm-9 no-padding no-float">
			...
			<div class="row form-group">
				<div class="col-sms-6 col-sm-6">
					<label><?php echo __( 'Kode Negara', 'trav' ) ?></label>
					<div class="selector">
			...
			<hr>
			<h2><?php echo __( 'Upload Foto Profil', 'trav' ) ?></h2>
			<div class="row form-group">
				<div class="col-sms-12 col-sm-6">
					<div class="fileinput full-width small-box">
						<input name="photo" type="file" class="input-text" data-placeholder="<?php echo esc_attr__( 'Pilih Gambar/s', 'trav' ); ?>" accept="image/*">
					</div>
				</div>
				<div class="col-sms-12 col-sm-6">
			...
			<hr>
			<h2><?php echo __( 'Ceritakan sedikit tentang kamu', 'trav' ) ?></h2>
			<div class="form-group">
				<textarea name="description" rows="5" class="input-text full-width" placeholder="<?php echo esc_attr__( 'Ceritakan tentang Kamu disini', 'trav' ); ?>"><?php echo esc_textarea( $user_info['description'] ); ?></textarea>
			</div>
			<div class="from-group">
				<button type="submit" class="btn-medium col-sms-6 col-sm-4"><?php echo __( 'Update Data', 'trav' ) ?></button>
			</div>
		</div>
	</form>
</div>

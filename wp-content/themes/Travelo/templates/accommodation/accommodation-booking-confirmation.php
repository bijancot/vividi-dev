...
if ( ! empty( $deposit_rate ) && empty( $booking_data['deposit_paid'] ) ) {
    ...
    $ItemName = '';
    if ( $deposit_rate < 100 ) {
        $ItemName = sprintf( __( 'Deposit(%d%%) untuk ', 'trav' ), $deposit_rate );
    } else {
        $ItemName = __( 'Deposit untuk ', 'trav' );
    }
    $ItemName .= get_the_title( $acc_id ) . ' ' . get_the_title( $room_type_id ) . ' ' . $booking_data['rooms'] . __( 'Kamar', 'trav' );

    $payment_data = array();
    $payment_data['item_name'] = $ItemName;
    $payment_data['item_number'] = $acc_id . '-' . $room_type_id;
    $payment_data['item_desc'] = __( 'Dari', 'trav' ) . ' ' . $date_from . ' ' . __( 'Sampai', 'trav' ) . ' ' . $date_to . ' ' . get_the_title( $acc_id ) . ' ' . get_the_title( $room_type_id ) . ' ' . $booking_data['rooms'] . __( 'Kamar', 'trav' );

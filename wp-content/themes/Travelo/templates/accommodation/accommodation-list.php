...
foreach( $acc_list as $acc_id ) {
    ...
    if ( $current_view == 'block' ) { ?>
        ...
        <article class="box">
            <figure>
                <a title="<?php _e( 'Galerry', 'trav' ); ?>" class="hover-effect popup-gallery" data-post_id="<?php echo esc_attr( $acc_id );?>" href="#"><?php echo get_the_post_thumbnail( $acc_id, 'gallery-thumb' ); ?></a>
            ...
            <div class="details">
                <a title="<?php _e( 'Rincian', 'trav' ); ?>" href="<?php echo esc_url( $url ); ?>" class="pull-right button uppercase"><?php _e( 'Pilih', 'trav' ); ?></a>
                ...
                <label class="price-wrapper"><span class="price-per-unit"><?php echo esc_html( trav_get_price_field( $avg_price ) ); ?></span><?php _e( 'Per Malam', 'trav' ) ?></label>
            </div>
        </article>
    ...
    <?php } elseif ( $current_view == 'grid' ) { ?>
        ...
        <article class="box">
            <figure>
                <a title="<?php _e( 'Gallery', 'trav' ); ?>" class="hover-effect popup-gallery" data-post_id="<?php echo esc_attr( $acc_id );?>" href="#"><?php echo get_the_post_thumbnail( $acc_id, 'gallery-thumb' ); ?></a>
                <?php if ( ! empty( $discount_rate ) ) { ?>
                    <span class="discount"><span class="discount-text"><?php echo esc_html( $discount_rate . '%' . ' ' . __( 'Discount', 'trav' ) ); ?></span></span>
                <?php } ?>
            </figure>
            <div class="details">
                ...
                <h4 class="box-title"><a href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( get_the_title( $acc_id ) );?></a><?php echo trav_acc_get_star_rating( $acc_id ); ?><small><?php echo esc_html( trav_acc_get_city( $acc_id ) . ' ' . trav_acc_get_country( $acc_id ) ); ?></small></h4>
                <div class="feedback">
                    <div data-placement="bottom" data-toggle="tooltip" class="five-stars-container" title="<?php echo esc_attr( $review . ' ' . __( 'Bintang', 'trav' ) ) ?>"><span style="width: <?php echo esc_html( $review / 5 * 100 ) ?>%;" class="five-stars"></span></div>

...
foreach( $car_list as $car_id ) {
	...
	$time = array( 'everytime' => __( 'Setiap Saat', 'trav' ), 'morning' => __( 'Mulai 04.00 WIB', 'trav' ), 'afternoon' => __( 'Pukul 22.00 WIB', 'trav' ) );
		...
		<article class="box">
            ...
            <div class="details">
				<?php if ( ! empty( $price ) && is_numeric( $price ) ) { ?>
					<span class="price" style="color: #09477E"><small><?php _e( 'per day', 'trav' ) ?></small><?php echo esc_html( trav_get_price_field( $price ) ); ?></span>
				<?php } ?>
<!--                <h4 class="box-title">--><?php //echo esc_html( $car_type ) ?><!--<small>--><?php //echo esc_html( get_the_title( $car_id ) ); ?><!--</small></h4>-->
                ...
                <p class="mile"><span class="skin-color"><?php _e( 'KM Pemakaian:', 'trav' ); ?></span> <?php echo ( isset( $car_mileage ) && is_numeric( $car_mileage ) )?__( 'sampai', 'trav' ) . ' ' . $car_mileage . __( 'KM', 'trav' ):__( 'Wajar', 'trav' ); ?></p>
                <div class="action">
                    <a class="button btn-small full-width" href="<?php echo esc_url( $url ); ?>"><?php _e( 'SEWA', 'trav' ); ?></a>
                </div>
            </div>
        </article>
		...
		<article class="box">
			...
			<div class="details col-xs-9 clearfix">
                ...
                <div class="col-xs-6 col-sm-4 character">
                    <dl class="">
                       	<dt class="skin-color"><?php _e( 'Km Sewa', 'trav' ); ?></dt><dd><?php echo ( isset( $car_mileage ) && is_numeric( $car_mileage ) ) ? $car_mileage . __( 'miles', 'trav' ) : __( 'unlimited', 'trav' ); ?></dd>
                       	<dt class="skin-color"><?php _e( 'Penjemputan', 'trav' ); ?></dt><dd><?php echo ( isset( $car_pick_up_time ) && isset( $time[$car_pick_up_time] ) ) ? $time[$car_pick_up_time] : __( 'anytime', 'trav' ); ?></dd>
                        <?php if ( isset( $car_location ) ) { ?>
                        	<dt class="skin-color"><?php _e( 'lokasi', 'trav' ); ?></dt><dd><?php echo $car_location; ?></dd>
                        <?php } ?> 
                    </dl>
                </div>
                <div class="action col-xs-6 col-sm-6">

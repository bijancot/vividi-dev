...
if ( ! function_exists( 'trav_get_car_js_data' ) ) {
	function trav_get_car_js_data() {
		...
		//messages
		$car_data['msg_no_booking_page'] = __( 'Silakan atur halaman pemesanan di admin / Opsi Tema / Pengaturan Halaman', 'trav' );
		$car_data['msg_wrong_date_1'] = __( 'Masukkan tanggal Jemput dan Selesai Anda di kotak pencarian, Jangan Lupa LOGIN dulu', 'trav' );
		$car_data['msg_wrong_date_2'] = __( 'Mohon pilih tanggal jemput.', 'trav' );
		$car_data['msg_wrong_date_3'] = __( 'Mohon pilih tanggal selesai.', 'trav' );
		$car_data['msg_wrong_date_4'] = __( 'Mohon pilih waktu jemput.', 'trav' );
		$car_data['msg_wrong_date_5'] = __( 'Mohon pilih waktu selesai.', 'trav' );
		$car_data['msg_wrong_date_6'] = __( 'Mohon pilih lokasi jemput.', 'trav' );
		$car_data['msg_wrong_date_7'] = __( 'Mohon pilih lokasi selesai.', 'trav' );
		$car_data['msg_wrong_date_8'] = __( 'Tanggal selesai kamu sebelum tanggal jemput. Mohon periksa kembali.', 'trav' );
		$car_data['msg_wrong_date_9'] = __( 'Waktu jemput salah, Mohon periksa kembali.', 'trav' );
		$car_data['msg_wrong_date_10'] = __( 'Pencarian salah. Silakan periksa lagi.', 'trav' );
...
if ( ! function_exists( 'trav_car_check_availability' ) ) {
	function trav_car_check_availability( $car_id, $from_date, $to_date, $except_booking_no=0, $pin_code=0 ) {
		if ( empty( $car_id ) || 'car' != get_post_type( $car_id ) ) return esc_html__( 'ID Kendaraan tidak berlaku.', 'trav' ); //invalid data
		...
		if ( ! trav_strtotime( $from_date ) || ! trav_strtotime( $to_date ) || ( trav_strtotime( $from_date ) > trav_strtotime( $to_date ) ) ) {
			return esc_html__( 'Tanggal tidak berlaku. Periksa tanggal pemesanan Kamu lagi.', 'trav' ); //invalid data
		}
		...
		foreach ( $period as $dt ) {
			...
			if ( $cars == 0 ) {
				return esc_html__( 'Kendaraan tidak tersedia. Silakan lihat lagi tanggal atau tipe lainnya.', 'trav' );
			}
		}
		return true;
	}
...
if ( ! function_exists( 'trav_car_booking_before' ) ) {
	function trav_car_booking_before() {
		...
		parse_str( urldecode( $_REQUEST['booking_data'] ), $raw_booking_data );
...
if ( ! function_exists( 'trav_car_deposit_payment_not_paid' ) ) {
	function trav_car_deposit_payment_not_paid( $booking_data ) {
		echo '<div class="alert alert-notice">' . __( 'Biaya lainnya belum Di terima.', 'trav' ) . '<span class="close"></span></div>';
	}
}

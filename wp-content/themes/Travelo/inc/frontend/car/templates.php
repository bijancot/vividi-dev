...
<!--                            <h4 class="box-title">--><?php //echo esc_html( $car_type ) ?><!--<small>--><?php //echo esc_html( get_the_title( $car_id ) ); ?><!--</small></h4>-->
                        ...
                        <div class="amenities">
                            ...
                            <?php if ( isset( $car_mileage ) && is_numeric( $car_mileage ) ) { ?>
                            	<p class="mile"><span class="skin-color"><?php _e( 'KM Sewa:', 'trav' ); ?></span> <?php echo __( 'up to', 'trav' ) . $car_mileage . __( 'miles', 'trav' ); ?></p>
                            <?php } ?>
                            <div class="action">
                                <a class="button btn-small full-width" href="<?php echo esc_url( get_permalink( $car_id ) ); ?>"><?php _e( 'SEWA', 'trav' ); ?></a>
                            </div>

					<?php } elseif ( $list_style == "style2" ) { ?>

                            <a title="View all" href="<?php echo esc_url( get_permalink( $car_id ) ); ?>" class="pull-right button btn-mini uppercase"><?php _e( 'SEWA', 'trav' ); ?></a>
                            <h4 class="box-title"><?php echo esc_html( $car_type ) ?></h4>
                            <label class="price-wrapper">
                                <span class="price-per-unit"><?php echo esc_html( trav_get_price_field( $price ) ); ?></span><?php _e( 'Per Hari', 'trav' ) ?>
                            </label>
			...
			<article class="box">
                ...
                <div class="details col-xs-9 clearfix">
                    ...
                    <div class="col-xs-6 col-sm-2 character">
                        <dl class="">
                        	<?php if ( isset( $car_mileage ) && is_numeric( $car_mileage ) ) { ?>
                            	<dt class="skin-color"><?php _e( 'Km Sewa', 'trav' ); ?></dt><dd><?php echo $car_mileage . __( 'miles', 'trav' ); ?></dd>
                            <?php } ?>                            
                        </dl>
                    </div>
                    <div class="action col-xs-6 col-sm-2">
                        <span class="price"><small><?php _e( 'pPer Hari', 'trav' ) ?></small><?php echo esc_html( trav_get_price_field( $price ) ); ?></span>
                        <a class="button btn-small full-width" href="<?php echo esc_url( get_permalink( $car_id ) ); ?>"><?php _e( 'Sewa', 'trav' ); ?></a>
                    </div>
                </div>
            </article>

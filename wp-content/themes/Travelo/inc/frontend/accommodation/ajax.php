...
if ( ! function_exists( 'trav_ajax_acc_get_month_vacancies' ) ) {
    function trav_ajax_acc_get_month_vacancies() {
        ...
        if ( ! isset( $_POST['year'] ) || ! isset( $_POST['month'] ) || ! isset( $_POST['accommodation_id'] ) ) {
            ...
            $result_json['result'] = __( 'Salah memasukan data.', 'trav' );
...
if ( ! function_exists( 'trav_ajax_acc_get_available_rooms' ) ) {
    function trav_ajax_acc_get_available_rooms() {
        ...
        if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['accommodation_id'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . sanitize_text_field( $_POST['accommodation_id'] ) ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, Kehadiran kamu tidak terverifikasi.', 'trav' );
...
if ( ! function_exists( 'trav_ajax_acc_submit_review' ) ) {
    function trav_ajax_acc_submit_review() {
        ...
        if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . $_POST['post_id'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, Salah memasukan data.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( ! $booking_data = trav_acc_get_booking_data( $data['booking_no'], $data['pin_code'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Salah kode booking atau PIN.', 'trav' );
            wp_send_json( $result_json );
        }

        if ( ! is_array( $booking_data ) || $booking_data['status'] == 0 || trav_acc_org_id($data['post_id']) != $booking_data['accommodation_id'] ) {
            $result_json['success'] = 0;
            $result_json['title'] = __( 'Maaf, Kamu tidak dapat memberikan ranking.', 'trav' );
            $result_json['result'] = __( 'Kamu telah membatalkan pemesanan, jadi tidak bisa memberikan ranking.', 'trav' );
            wp_send_json( $result_json );
        }

        if ( trav_strtotime( $booking_data['date_to'] ) > trav_strtotime(date("Y-m-d")) ) {
            $result_json['success'] = 0;
            $result_json['title'] = __( 'Maaf, Kamu tidak dapat memberikan ranking sebelum berwisata.', 'trav' );
            $result_json['result'] = __( 'Kamu dapat meninggalkan ulasan setelah berwisata.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( ! $review_data = $wpdb->get_row( sprintf( 'SELECT * FROM ' . TRAV_REVIEWS_TABLE . ' WHERE booking_no=%d AND pin_code=%d', $data['booking_no'], $data['pin_code'] ), ARRAY_A ) ) {
            if ( $wpdb->insert( TRAV_REVIEWS_TABLE, $data ) ) {
                $result_json['success'] = 1;
                $result_json['title'] = __( 'Terima kasih! Ulasan kamu telah sukses terkirim.', 'trav' );
                $result_json['result'] = __( 'Kamu dapat mengubah ulasan kapan saja.', 'trav' );
            } else {
                $result_json['success'] = 0;
                $result_json['title'] = __( 'Terima kasih! Ulasan kamu telah sukses terkirim.', 'trav' );
                $result_json['result'] = __( 'Silakan coba lagi setelah beberapa saat.', 'trav' );
            }
        } else {
...
if ( ! function_exists( 'trav_ajax_acc_add_to_wishlist' ) ) {
    function trav_ajax_acc_add_to_wishlist() {
        ...
        if ( ! is_user_logged_in() ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Silakan masuk untuk memperbarui daftar keinginan kamu.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( isset( $_POST['remove'] ) ) {
            ...
            if ( update_user_meta( $user_id, 'wishlist', $wishlist ) ) {
                $result_json['success'] = 1;
                $result_json['result'] = __( 'Properti ini telah berhasil dihapus dari daftar keinginan kamu.', 'trav' );
            } else {
                $result_json['success'] = 0;
                $result_json['result'] = __( 'Maaf, Terjadi kesalahan saat memperbarui daftar keinginan.', 'trav' );
            }
        } else {
            //add
            ...
            if ( ! in_array( $new_item_id, $wishlist) ) {
                ...
                $result_json['success'] = 1;
                $result_json['result'] = __( 'Sudah ada di daftar keinginan kamu.', 'trav' );
            }
        }
        wp_send_json( $result_json );
    }
...
if ( ! function_exists( 'trav_ajax_acc_submit_booking' ) ) {
    function trav_ajax_acc_submit_booking() {
        ...
        if ( ! isset( $_POST['transaction_id'] ) || ! isset( $_SESSION['booking_data'][$_POST['transaction_id']] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, beberapa kesalahan terjadi pada validasi data input.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . $booking_data['room_type_id'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, kehadiran Kamu tidak diverifikasi.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( isset( $trav_options['vld_captcha'] ) && ! empty( $trav_options['vld_captcha'] ) ) {
            if ( ! isset( $_POST['security_code'] ) || $_POST['security_code'] != $_SESSION['security_code'] ) {
                $result_json['success'] = 0;
                $result_json['result'] = __( 'Kesalahan kode keamanan. Silakan periksa kode keamanan Anda lagi.', 'trav' );
                wp_send_json( $result_json );
            }
        }
        ...
        if ( isset( $trav_options['vld_credit_card'] ) && ! empty( $trav_options['vld_credit_card'] ) ) {
            if ( ! isset( $_POST['cc_type'] ) || ! isset( $_POST['cc_holder_name'] ) || ! isset( $_POST['cc_number'] ) || ! isset( $_POST['cc_exp_month'] ) || ! isset( $_POST['cc_exp_year'] ) || ! trav_cc_validation( $_POST['cc_type'], $_POST['cc_holder_name'], $_POST['cc_number'], $_POST['cc_exp_month'], $_POST['cc_exp_year'] ) ) {
                $result_json['success'] = 0;
                $result_json['result'] = __( 'Validasi VCC Gagal.', 'trav' );
                wp_send_json( $result_json );
            }
        }
...
//        $booking_no = mt_rand( 1000, 9999 );
        ...
        if ( ! $room_price_data || ! is_array( $room_price_data ) ) {
            $result_json['success'] = -1;
            $result_json['result'] = __( 'Maaf, kamar yang Kmau pesan sekarang baru saja diambil oleh pelanggan lain. Silakan lihat lagi.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( $wpdb->insert( TRAV_ACCOMMODATION_BOOKINGS_TABLE, $data ) ) {
            ...
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, Terjadi kesalahan saat menambahkan pemesanan.', 'trav' );
        }
...
if ( ! function_exists( 'trav_ajax_acc_cancel_booking' ) ) {
    function trav_ajax_acc_cancel_booking() {
        ...
        if ( false === $result ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, beberapa kesalahan terjadi saat pembaruan.', 'trav' );
        } else {
            $result_json['success'] = 1;
            $result_json['result'] = __( 'Pembatalan pesanan Sukses.', 'trav' );
...
if ( ! function_exists( 'trav_ajax_acc_check_room_availability' ) ) {
    function trav_ajax_acc_check_room_availability() {
        ...
        if ( ! isset( $_POST['date_from'] ) || ! isset( $_POST['date_to'] ) || trav_strtotime( $_POST['date_from'] ) >= trav_strtotime( $_POST['date_to'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Interval Tanggal Salah. Silakan periksa tanggal Anda.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( ! $booking_data = trav_acc_get_booking_data( $booking_no, $pin_code ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Kesalahan Kode Booking atau PIN.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( ! $room_price_data || ! is_array( $room_price_data ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Kamar tidak tersedia untuk tanggal, kamar, dan orang yang dipilih. Silakan lihat lagi di kolom pemesanan.', 'trav' );
            wp_send_json( $result_json );
        } else {
            ...
            $return_html .= '<dt class="feature">' . esc_html( trav_get_day_interval( $booking_data['date_from'], $booking_data['date_to'] ) ) . ' ' . __( 'Malam', 'trav') . ':</dt><dd class="value">' . esc_html( trav_get_price_field( $room_price_data['total_price'] ) ) . '</dd>';
            if ( ! empty( $tax_rate ) ) :
                $return_html .= '<dt class="feature">' . __( 'Pajak', 'trav') . ':</dt><dd class="value">' . esc_html( trav_get_price_field( $tax ) ) . '</dd>';
            endif;
            $return_html .= '<dt class="feature">' . __( 'Total Harga', 'trav') . '</dt><dd>' . esc_html( trav_get_price_field( $total_price ) ) . '</dd>';
            $return_html .= '</dl>';
...
if ( ! function_exists( 'trav_ajax_acc_update_booking_date' ) ) {
    function trav_ajax_acc_update_booking_date() {
        if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['booking_no'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'booking-' . $_POST['booking_no'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Maaf, Kehadiran anda tidak terverifikasi.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        if ( false === $result ) {
            ...
            $result_json['success'] = 1;
            $result_json['result'] = __( 'Pemesanan Kamu berhasil diperbarui.', 'trav' );
        }
        wp_send_json( $result_json );
    }
}

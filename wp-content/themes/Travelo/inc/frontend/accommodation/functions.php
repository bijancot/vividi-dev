...
if ( ! function_exists( 'trav_acc_get_star_rating' ) ) {
    function trav_acc_get_star_rating( $acc_id ) {
		...
		if ( ! empty ( $accommodation_type ) && ! is_wp_error( $accommodation_type ) ) {
            $acc_type_name =  $accommodation_type[0]->name;
        }
        if ( ! empty( $hotel_star_rating ) ) {
            $result = ' <div title="' . $hotel_star_rating . '-' . __( 'Bintang', 'trav') . ' ' . $acc_type_name . '" class="five-stars-container no-back-star" data-toggle="tooltip" data-placement="bottom"><span class="five-stars" style="width: ' . ( $hotel_star_rating / 5 * 100 ) . '%;"></span></div>';
        }
        return $result;
    }
}
...
if ( ! function_exists( 'trav_acc_send_confirmation_email' ) ) {
    function trav_acc_send_confirmation_email( $booking_no, $booking_pincode, $type='new', $subject='', $description='' ) {
        ...
        if ( ! empty( $booking_data ) ) {
            ...
            $booking_valid_until = $a[0].", ".$b[0]." ".$b[1]." ".$b[2]." ".$b[3];
...
if ( ! function_exists( 'trav_get_acc_js_data' ) ) {
    function trav_get_acc_js_data() {
        ...
        $review_labels = array(
                            '4.75' => __('Luar Biasa', 'trav'),
                            '4.5' => __('Hebat', 'trav'),
                            '4' => __('Sangat Baik', 'trav'),
                            '3.5' => __('Baik', 'trav'),
                            '3' => __('Cukup', 'trav'),
                            '0' => __('Kurang', 'trav'),
        );
        ...
        //messages
        $acc_data['msg_no_booking_page'] = __( 'Silakan atur halaman pemesanan akomodasi di admin / Opsi Tema / Pengaturan Halaman', 'trav' );
        $acc_data['msg_wrong_date_1'] = __( 'Masukkan tanggal check-in dan check-out Kamu di kotak pencarian dan klik tombol Cari Sekarang', 'trav' );
        $acc_data['msg_wrong_date_2'] = __( 'Silakan pilih tanggal Check-in.', 'trav' );
        $acc_data['msg_wrong_date_3'] = __( 'Silakan pilih tanggal Check-out.', 'trav' );
        $acc_data['msg_wrong_date_4'] = __( 'Tanggal check-out Kamu sebelum tanggal check-in. Lihat dan coba lagi.', 'trav' );
        $acc_data['msg_wrong_date_5'] = sprintf( __( 'Minimal menginap di hotel ini %d Malam. Lihat dan coba lagi.', 'trav' ), $minimum_stay );
        $acc_data['msg_wrong_date_6'] = __( 'Tanggal check-in salah. Silakan periksa lagi.', 'trav' );
        $acc_data['msg_wrong_date_7'] = __( 'Bidang pencarian salah. Silakan periksa lagi.', 'trav' );

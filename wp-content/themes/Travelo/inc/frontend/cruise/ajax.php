...
/*
 * Handle get cruise month schedules ajax request.
 */
if ( ! function_exists( 'trav_ajax_cruise_get_month_schedules' ) ) {
	function trav_ajax_cruise_get_month_schedules() {
		$result_json = array( 'success' => 0, 'result' => '' );
		//validation
		if ( ! isset( $_POST['year'] ) || ! isset( $_POST['month'] ) || ! isset( $_POST['cruise_id'] ) ) {
		...
		//initiate variables
		$cruise_id = sanitize_text_field( $_POST['cruise_id'] );
		$year = sanitize_text_field( $_POST['year'] );
		$month = sanitize_text_field( $_POST['month'] );
		$schedules = trav_cruise_get_month_schedules( $cruise_id, $year, $month );

		$result_json['success'] = 1;
		$result_json['result'] = $schedules;
		wp_send_json( $result_json );
	}
}

/*
 * Handle ajax request to get matched cabins to given data.
 */
if ( ! function_exists( 'trav_ajax_cruise_get_available_cabins' ) ) {
    function trav_ajax_cruise_get_available_cabins() {
        /*if ( isset( $_POST[ 'lang' ] ) ) {
            global $sitepress;
        ...
        if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['cruise_id'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . sanitize_text_field( $_POST['cruise_id'] ) ) ) {
            $result_json['success'] = 0;
        ...
        $cabins = ( isset( $_POST['cabins'] ) && is_numeric( $_POST['cabins'] ) ) ? sanitize_text_field( $_POST['cabins'] ) : 1;
        ...
        $duration = ( isset( $_POST['duration'] ) && is_numeric( $_POST['duration'] ) ) ? sanitize_text_field( $_POST['duration'] ) : 1;

        if ( isset( $_POST['cruise_id'] ) && isset( $_POST['date_from'] ) && trav_strtotime( $_POST['date_from'] ) && ( ( time()-(60*60*24) ) < trav_strtotime( $_POST['date_from'] ) ) ) {
            ...
            $return_value = trav_cruise_get_available_cabins( $cruise_id, $_POST['date_from'], $duration, $cabins, $adults, $kids, $child_ages, $except_booking_no, $pin_code );
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_submit_booking' ) ) {
    function trav_ajax_cruise_submit_booking() {
        ...
        $booking_fields = array( 'cruise_id', 'cabin_type_id', 'cabins', 'adults', 'kids', 'child_ages', 'total_price', 'cabin_price', 'tax', 'currency_code', 'exchange_rate', 'deposit_price', 'date_from', 'date_to', 'duration', 'created', 'booking_no', 'pin_code', 'status', 'discount_rate' );
        $booking_data = array();
        ...
        if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . $booking_data['cabin_type_id'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Sorry, your nonce did not verify.', 'trav' );
            wp_send_json( $result_json );
        }
        ...
        $latest_booking_id = $wpdb->get_var( 'SELECT id FROM ' . TRAV_CRUISE_BOOKINGS_TABLE . ' ORDER BY id DESC LIMIT 1' );
        ...
        $cabin_price_data = trav_cruise_get_cabin_price_data( $data['cruise_id'], $data['cabin_type_id'], $booking_data['date_from'], $booking_data['duration'], $data['cabins'], $data['adults'], $data['kids'], $data['child_ages'] );
        if ( ! $cabin_price_data || ! is_array( $cabin_price_data ) ) {
            $result_json['success'] = -1;
            $result_json['result'] = __( 'Sorry, The cabin you are booking now is just taken by another customer. Please have another look.', 'trav' );
            wp_send_json( $result_json );
        }

        do_action( 'trav_cruise_add_booking_before', $data );

        // save default language cruise and cabin type
        $data['cruise_id'] = trav_cruise_org_id( $data['cruise_id'] );
        $data['cabin_type_id'] = trav_cabin_org_id( $data['cabin_type_id'] );

        // add to db
        if ( $wpdb->insert( TRAV_CRUISE_BOOKINGS_TABLE, $data ) ) {
            ...
            if ( $is_payment_enabled ) {
                if ( trav_is_woo_enabled() ) {
                    // woocommerce
                    do_action( 'trav_woo_add_cruise_booking', $data );
            ...
            do_action( 'trav_cruise_add_booking_after', $data );
        } else {
            $result_json['success'] = 0;
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_cancel_booking' ) ) {
    function trav_ajax_cruise_cancel_booking() {
        ...
        $pin_code = sanitize_text_field( $_POST['pin_code'] );
        $result = trav_cruise_update_booking( $edit_booking_no, $pin_code, array( 'status' => 0 ), 'cancel' );
        if ( false === $result ) {
            $result_json['success'] = 0;
 ...
 * update booking date
 */
if ( ! function_exists( 'trav_ajax_cruise_update_booking_date' ) ) {
    function trav_ajax_cruise_update_booking_date() {
        ...
        if ( ! $booking_data = trav_cruise_get_booking_data( $booking_no, $pin_code ) ) {
            $result_json['success'] = 0;
        ...
        $booking_data['duration'] = sanitize_text_field( $_POST['duration'] );
        $booking_data['cabins'] = sanitize_text_field( $_POST['cabins'] );
        ...
        $date_to_obj = new DateTime( '@' . trav_strtotime( $booking_data['date_from'] .' + ' . $booking_data['duration'] . ' days' ) );
        $booking_data['date_to'] = $date_to_obj->format( "Y-m-d" );

        $cabin_price_data = trav_cruise_get_cabin_price_data( $booking_data['cruise_id'], $booking_data['cabin_type_id'], $booking_data['date_from'], $booking_data['duration'], $booking_data['cabins'], $booking_data['adults'], $booking_data['kids'], $booking_data['child_ages'], $booking_no, $pin_code );

        if ( ! $cabin_price_data ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'The cabin is not available for the selected date, cabins and person. Please have another look at booking fields.', 'trav' );
            wp_send_json( $result_json );
        }

        $tax_rate = get_post_meta( $booking_data['cruise_id'], 'trav_cruise_tax_rate', true );
        $tax = 0;
        if ( ! empty( $tax_rate ) ) $tax = $tax_rate * $cabin_price_data['total_price'] / 100;
        $total_price_incl_tax = $cabin_price_data['total_price'] + $tax;
        ...
        $result = trav_cruise_update_booking( $booking_no, $pin_code, $booking_data, 'update' );
        if ( false === $result ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Sorry, some error occurred on update.', 'trav' );
        } else {
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_change_cabin' ) ) {
    function trav_ajax_cruise_change_cabin() {
        if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['booking_no'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'booking-' . $_POST['booking_no'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Sorry, your nonce did not verify.', 'trav' );
            wp_send_json( $result_json );
        ...
        $booking_data['cabin_type_id'] = sanitize_text_field( trav_cabin_org_id( $_POST['cabin_type_id'] ) );
        $cabin_price_data = trav_cruise_get_cabin_price_data( $booking_data['cruise_id'], $booking_data['cabin_type_id'], $booking_data['date_from'], $booking_data['duration'], $booking_data['cabins'], $booking_data['adults'], $booking_data['kids'], $booking_data['child_ages'], $booking_no, $pin_code );
...
/*
 * Check cabin availability for updated date on thank you Page
 */
if ( ! function_exists( 'trav_ajax_cruise_check_cabin_availability' ) ) {
    function trav_ajax_cruise_check_cabin_availability() {
        if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['booking_no'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'booking-' . $_POST['booking_no'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Sorry, your nonce did not verify.', 'trav' );
            wp_send_json( $result_json );
        ...
        if ( ! isset( $_POST['date_from'] ) || ! isset( $_POST['duration'] ) ) {
            $result_json['success'] = 0;
        ...
        $booking_data['date_from'] = sanitize_text_field( $_POST['date_from'] );
        $booking_data['duration'] = sanitize_text_field( $_POST['duration'] );
        ...
        if ( ! $cabin_price_data || ! is_array( $cabin_price_data ) ) {
            ...
            $tax = 0;
            if ( ! empty( $tax_rate ) ) $tax = $tax_rate * $cabin_price_data['total_price'] / 100;
            $total_price = $cabin_price_data['total_price'] + $tax;

            $return_html = '<dl class="other-details">';
            $return_html .= '<dt class="feature">' . esc_html( trav_get_day_interval( $booking_data['date_from'], $booking_data['date_to'] ) ) . ' ' . __( 'night Stay', 'trav') . ':</dt><dd class="value">' . esc_html( trav_get_price_field( $cabin_price_data['total_price'] ) ) . '</dd>';
            if ( ! empty( $tax_rate ) ) :
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_submit_review' ) ) {
    function trav_ajax_cruise_submit_review() {
        global $wpdb;
        ...
        if ( ! $booking_data = trav_cruise_get_booking_data( $data['booking_no'], $data['pin_code'] ) ) {
            $result_json['success'] = 0;
            $result_json['result'] = __( 'Wrong Booking Number and Pin Code.', 'trav' );
            wp_send_json( $result_json );
        }

        if ( ! is_array( $booking_data ) || $booking_data['status'] == 0 || trav_cruise_org_id($data['post_id']) != $booking_data['cruise_id'] ) {
            $result_json['success'] = 0;
        ...
        $data['post_id'] = $booking_data['cruise_id'];
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_add_to_wishlist' ) ) {
    function trav_ajax_cruise_add_to_wishlist() {
        $result_json = array( 'success' => 0, 'result' => '' );
        ...
        $new_item_id = sanitize_text_field( trav_cruise_org_id( $_POST['cruise_id'] ) );
        ...
        if ( isset( $_POST['remove'] ) ) {
            ...
            if ( update_user_meta( $user_id, 'wishlist', $wishlist ) ) {
                $result_json['success'] = 1;
                $result_json['result'] = __( 'This cruise has removed from your wishlist successfully.', 'trav' );
            } else {
                $result_json['success'] = 0;
            ...
            if ( ! in_array( $new_item_id, $wishlist) ) {
                ...
                if ( update_user_meta( $user_id, 'wishlist', $wishlist ) ) {
                    $result_json['success'] = 1;
                    $result_json['result'] = __( 'This cruise has added to your wishlist successfully.', 'trav' );
                } else {
                    $result_json['success'] = 0;
                    $result_json['result'] = __( 'Sorry, An error occurred while update wishlist.', 'trav' );
                }
 ...
 */
if ( ! function_exists( 'trav_ajax_cruise_get_more_reviews' ) ) {
    function trav_ajax_cruise_get_more_reviews() {
        $cruise_id = sanitize_text_field( $_POST['cruise_id'] );
        $last_no = sanitize_text_field( $_POST['last_no'] );
        $per_page = 10;
        $review_count = trav_get_review_html( $cruise_id, $last_no, $per_page );
        exit();
    }
}
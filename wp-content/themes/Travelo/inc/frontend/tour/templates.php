	...
	function trav_tour_get_single_schedule_html( $tour_id, $st_id, $schedule ) {
		...
		<div class="intro small-box table-wrapper full-width hidden-table-sms">
			<div class="col-sm-4 table-cell features">
				<table>
					<tr><td><label><?php _e( 'Lokasi Tour', 'trav' ) ?>:</label></td><td><?php echo esc_html( $location ) ?></td></tr>
					...
					<?php endif; ?>
					<tr><td><label><?php _e( 'Durasi', 'trav' ) ?>:</label></td><td><?php echo esc_html( $default_data['duration'] ) ?></td></tr>
					<tr><td><label><?php _e( 'Masih Tersedia', 'trav' ) ?>:</label></td><td class="available-seats"><?php echo esc_html( $default_data['available_seat'] ) ?></td></tr>

					<?php if ( ! empty( $multi_book ) ) { ?>
						<tr><td><label><?php _e( 'Harga Dewasa', 'trav' ) ?>:</label></td><td class="adult-price"><?php echo esc_html( trav_get_price_field( $default_data['price'] ) ) ?></td></tr>
						<?php if ( ! empty( $default_data['child_price'] ) && ( (float) $default_data['child_price'] ) != 0 ) { ?>
							<tr><td><label><?php _e( 'Harga Anak', 'trav' ) ?>:</label></td><td class="child-price"><?php echo esc_html( trav_get_price_field( $default_data['child_price'] ) ) ?></td></tr>
						<?php } ?>
					<?php } else { ?>
						<tr><td><label><?php _e( 'Harga', 'trav' ) ?>:</label></td><td class="adult-price"><?php echo esc_html( trav_get_price_field( $default_data['price'] ) ) ?></td></tr>
					<?php } ?>
				...
				<form method="get" action="<?php echo $tour_booking_page ?>" class="tour-booking-form">
					...
					<div class="detail-section-top row">
						<div class="st-details col-md-9 col-sm-8">
							...
							<div class="time"><i class="soap-icon-clock blue-color"></i><span><?php echo wp_kses_post( $st_time ) ?></span></div>
							<?php endif; ?>
						</div>
						<div class="price-details col-md-3 col-sm-4">
							<h3 class="price" style="color: #09477E">
							...
							<?php if ( empty( $multi_book ) && empty( $repeated ) ) : ?>
								<button title="book now" class="button btn-small full-width text-center btn-book-now <?php echo empty( $default_data['available_seat'] ) ? 'no-display' : '' ?>"><?php _e( "PESAN", "trav" ); ?></button>
							...
							<div class="row">
								<?php if ( ! empty( $repeated ) ) : ?>
									<div class="col-md-4 col-sm-6">
										<label><?php _e( 'TERSEDIA','trav' ); ?></label>
								...
								<div class="col-md-4 pull-right">
									...
									<div class="row">
                                        ...
                                        <?php } else { ?>
                                            <button style="background-color: #E35403" data-animation-duration="1" data-animation-type="bounce" class="btn-book-now full-width animated bounce <?php echo empty( $default_data['available_seat'] ) ? 'no-display' : '' ?>" type="submit"><?php _e( "PESAN", "trav" ); ?></button>
                                        <?php } ?>
											<h4 class="sold-out <?php echo empty( $default_data['available_seat'] ) ? '' : 'no-display' ?>"><?php echo __( 'Habis', 'trav' ) ?></h4>
											<h4 class="exceed-persons no-display"><?php echo __( 'Kuota Tidak Cukup', 'trav' ) ?></h4>
										</div>
		...
		if ( $list_style == "style1" ) { ?>
			...
			<article class="box">
				<figure <?php echo wp_kses_post( $animation ) ?>>
					<a title="<?php _e( 'Gallery', 'trav' ); ?>" class="hover-effect popup-gallery" data-post_id="<?php echo esc_attr( $tour_id );?>" href="#"><?php echo get_the_post_thumbnail( $tour_id, 'biggallery-thumb' ); ?></a>
				...
				<div class="details">
					...
					<div class="text-center">
						<div class="time">
							<i class="soap-icon-clock blue-color"></i>
							<span><?php echo esc_html( $duration ) ?></span>
						</div>
					<div class="action">
						<a title="<?php _e( 'Lihat Rincian', 'trav' ); ?>" class="button btn-small full-width" href="<?php echo esc_url( $url ); ?>"><?php _e( 'BPESAN', 'trav' ); ?></a>
					</div>
				</div>
			</article>
			...
			<article class="box">
				...
				<div class="details col-sm-7 col-md-8">
					<div>
						...
						<div>
							<span class="price"><small><?php _e( 'per orang', 'trav' ) ?></small><?php echo esc_html( trav_get_price_field( $min_price ) ); ?></span>
						</div>
					</div>
					<div>
						<?php echo wp_kses_post( $brief ); ?>
						<div>
							<a title="<?php _e( 'Lihat Rincian', 'trav' ); ?>" class="button btn-small full-width text-center" href="<?php echo esc_url( $url ); ?>"><?php _e( 'PESAN', 'trav' ); ?></a>
						</div>
					</div>
				</div>
			</article>

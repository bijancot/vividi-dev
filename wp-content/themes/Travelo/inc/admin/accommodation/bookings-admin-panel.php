...
class Trav_Acc_Booking_List_Table extends WP_List_Table {
    ...
    function column_customer_name( $item ) {
        ...
        $actions = array(
            'edit'      => sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'bookings', 'edit', $item['id'], 'Ubah' ),
            'delete'    => sprintf( $link_pattern, sanitize_text_field( $_REQUEST['post_type'] ), 'bookings', 'delete', $item['id'] . '&_wpnonce=' . wp_create_nonce( 'booking_delete' ) , 'Hapus' )
        );
    ...
    function get_columns() {
        $columns = array(
            ...
            'date_from'         => __( 'Check In', 'trav' ),
            'date_to'           => __( 'Check Out', 'trav' ),
            'accommodation_name'=> __( 'Properti', 'trav' ),
            'room_type_name'    => __( 'Tipe Kamar', 'trav' ),
            'rooms'             => __( 'Jumlah', 'trav' ),
    ...
    function prepare_items() {
        ...
        $sql = sprintf( 'SELECT Trav_Bookings.*, accommodation.ID as acc_id, accommodation.post_title as accommodation_name, room_type.ID as room_type_id, room_type.post_title as room_type_name FROM %1$s as Trav_Bookings
...
if ( ! function_exists( 'trav_acc_booking_add_menu_items' ) ) {
    function trav_acc_booking_add_menu_items() {
        ...
        $page = add_submenu_page( 'edit.php?post_type=accommodation', 'Accommodation Bookings', 'Pesanan Tamu', 'edit_accommodations', 'bookings', 'trav_acc_booking_render_pages' );
...
if ( ! function_exists( 'trav_acc_booking_render_list_page' ) ) {
    function trav_acc_booking_render_list_page() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e('LIST PESANAN PROPERTI', 'trav') ?> <a href="edit.php?post_type=accommodation&amp;page=bookings&amp;action=add" class="add-new-h2"><?php _e('Tambah Baru', 'trav') ?></a></h2>

            <?php 
            if ( isset( $_REQUEST['bulk_delete'] ) ) {
                echo '<div id="message" class="updated below-h2"><p>' . __('Hapus Pesanan', 'trav') . '</p></div>';
            }
            ?>
            ...
            <input type="text" id="date_from_filter" name="date_from" placeholder="<?php _e('Filter Check In', 'trav') ?>" value="<?php if ( ! empty( $_REQUEST['date_from'] ) ) echo esc_attr( $_REQUEST['date_from'] ); ?>">
            <input type="text" id="date_to_filter" name="date_to" placeholder="<?php _e('Filter Check Out', 'trav') ?>" value="<?php if ( ! empty( $_REQUEST['date_to'] ) ) echo esc_attr( $_REQUEST['date_to'] ); ?>">
            ...
            <a href="edit.php?post_type=accommodation&amp;page=bookings" class="button-secondary"><?php _e('Lihat Semua', 'trav') ?></a>
...
if ( ! function_exists( 'trav_acc_booking_render_manage_page' ) ) {
    function trav_acc_booking_render_manage_page() {
        ...
        if ( 'edit' == $_REQUEST['action'] ) {
            ...
            $sql = sprintf( 'SELECT Trav_Bookings.* , accommodation.post_title as accommodation_name, room_type.post_title as room_type_name FROM %1$s as Trav_Bookings
                            INNER JOIN %2$s as accommodation ON Trav_Bookings.accommodation_id=accommodation.ID
                            INNER JOIN %2$s as room_type ON Trav_Bookings.room_type_id=room_type.ID
        ...
        <div class="wrap">
            <?php $page_title = ( 'edit' == $_REQUEST['action'] ) ? __('Ubah Pemesanan Properti', 'trav') . '<a href="edit.php?post_type=accommodation&amp;page=bookings&amp;action=add" class="add-new-h2">' . __('Tambah Baru', 'trav') . '</a>' : __('Tambah baru Pemesanan Properti', 'trav'); ?>
            <h2><?php echo wp_kses_post( $page_title ); ?></h2>

            <?php 
            if ( isset( $_REQUEST['updated'] ) ) {
                echo '<div id="message" class="updated below-h2"><p>' . __('Simpan Pesanan', 'trav') . '</p></div>';
            }
            ?>
            ...
            <form method="post" onsubmit="return manage_booking_validateForm();">
                ...
                <div class="one-half">
                    <h3><?php _e('RINCIAN PEMESANAN', 'trav') ?></h3>
                    <table class="trav_admin_table trav_booking_manage_table">
                        <tr>
                            <th><?php _e('Nama Properti', 'trav') ?></th>
                            <td>
                        ...
                        <?php if ( trav_is_multi_currency() ) {?>
                            ...
                            <tr>
                                <th><?php _e('Total harga konversi', 'trav') ?></th>
                        ...
                        <?php if ( 'add' == $_REQUEST['action'] || ( ! empty( $booking_data['deposit_price'] ) && ! ( $booking_data['deposit_price'] == 0 ) ) ) { ?>
                            <tr>
                                <th><?php _e('Deposit dibayar', 'trav') ?></th>
                                <td>
                ...
                <div class="one-half">
                    ...
                    <table  class="trav_admin_table trav_booking_manage_table">
                        ...
                        <tr>
                            <th><?php _e('Pesan Spesial', 'trav') ?></th>
...
if ( ! function_exists( 'trav_acc_booking_delete_action' ) ) {
    function trav_acc_booking_delete_action() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            $sql = sprintf( 'SELECT Trav_Bookings.accommodation_id FROM %1$s as Trav_Bookings WHERE Trav_Bookings.id = %2$d' , TRAV_ACCOMMODATION_BOOKINGS_TABLE, $_REQUEST['booking_id'] );

...
class Trav_Acc_Vacancy_List_Table extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
			...
			'accommodation_name'  => __( 'Nama Properti', 'trav' ),
			'room_type_name' => __( 'Tipe Kamar', 'trav' ),
			'rooms'     => __( 'Allotment', 'trav' ),
			'price_per_room'  => __( 'Harga per Kamar', 'trav' ),
			'price_per_person'  => __( 'Harga per Orang', 'trav' )
		);
		return $columns;
	}
...
if ( ! function_exists( 'trav_acc_vacancy_add_menu_items' ) ) {
	function trav_acc_vacancy_add_menu_items() {
		$page = add_submenu_page( 'edit.php?post_type=accommodation', __('Atur Harga Kamar', 'trav'), __('Atur Harga', 'trav'), 'edit_accommodations', 'vacancies', 'trav_acc_vacancy_render_pages' );
...
if ( ! function_exists( 'trav_acc_vacancy_render_list_page' ) ) {
	function trav_acc_vacancy_render_list_page() {
		...
		<div class="wrap">
			<h2><?php _e('ATUR HARGA SEWA KAMAR', 'trav') ?> <a href="edit.php?post_type=accommodation&amp;page=vacancies&amp;action=add" class="add-new-h2"><?php _e('Tambah Baru', 'trav') ?></a></h2>
			<?php if ( isset( $_REQUEST['bulk_delete'] ) ) echo '<div id="message" class="updated below-h2"><p>' . __('Hapus Harga', 'trav') . '</p></div>'?>
			...
			<input type="text" id="date_filter" placeholder="<?php _e('Filter Waktu', 'trav') ?>" value="<?php echo isset($_REQUEST['date']) ? esc_attr( $_REQUEST['date'] ):'' ?>">
			<input type="button" name="vacancy_filter" id="vacancy-filter" class="button" value="<?php _e('Filter', 'trav') ?>">
			<a href="edit.php?post_type=accommodation&amp;page=vacancies" class="button-secondary"><?php _e('Lihat Semua', 'trav') ?></a>
...
if ( ! function_exists( 'trav_acc_vacancy_render_manage_page' ) ) {
	function trav_acc_vacancy_render_manage_page() {
		...
		if ( 'add' == $_REQUEST['action'] ) {
			$page_title = __('TAMBAH HARGA KAMAR', 'trav');
		} elseif ( 'edit' == $_REQUEST['action'] ) {
			$page_title = __('EDIT HARGA KAMAR', 'trav') . '<a href="edit.php?post_type=accommodation&amp;page=vacancies&amp;action=add" class="add-new-h2">' . __('Tambah Baru', 'trav') . '</a>';
		...
		<div class="wrap">
			...
			<?php if ( isset( $_REQUEST['updated'] ) ) echo '<div id="message" class="updated below-h2"><p>' . __('SIMPAN HARGA', 'trav') . '</p></div>'?>
			<form method="post" onsubmit="return manage_vacancy_validateForm();">
				...
				<table class="trav_admin_table trav_acc_vacancy_manage_table">
					...
					<tr>
						<th><?php _e('Allotment per Hari', 'trav') ?></th>
					...
					<tr>
						...
						<td><span><?php _e('Jika Kamu membiarkan bidang ini kosong, itu akan ditetapkan sebagai tanggal saat ini', 'trav') ?></span></td>
					</tr>
					<tr>
						<th><?php _e('Tanggal Akhir', 'trav') ?></th>
						...
						<td><span><?php _e('Biarkan kosong jika kamar ini tersedia sepanjang waktu', 'trav') ?></span></td>
					</tr>
					<tr>
						<th><?php _e('Harga Per Kamar', 'trav') ?></th>
					...
					<tr>
						<th><?php _e('Harga Per Orang', 'trav') ?></th>
					...
					<tr>
						<th><?php _e('Biaya Tambahan Anak?', 'trav') ?></th>
						<td>
							<label for="child_cost_yn">
								...
								<?php _e('Check list apabila dikenakan biaya', 'trav') ?>
							</label>
						</td>
					</tr>
					<tr class="child_cost">
						<th><?php _e('Harga Per Anak', 'trav') ?></th>
						<td>
							<table>
								<tr>
									<th><?php _e('Max Umur', 'trav') ?></th>
									<td><?php _e('Harga', 'trav') ?></td>
								</tr>
								<?php
									$child_price = array();
									...
									if ( ! empty( $child_price ) ) {
										...
										foreach ( $child_price as $age_price_pair ) {
											if ( is_array( $age_price_pair ) && ( count( $age_price_pair ) >=2 ) ) {
												...
												$html .= '<td><input type="number" name="child_price[' . $i . '][]" value="' . $age_price_pair[0] . '" min="1"></td>';
								...
								?>
									<tr class="clone-field">
										<td><input type="number" name="child_price[0][]" min="1"></td>
				...
				<input type="submit" class="button-primary" name="save" value="<?php _e('Simpan Harga', 'trav') ?>">
				<a href="edit.php?post_type=accommodation&amp;page=vacancies" class="button-secondary"><?php _e('Batal', 'trav') ?></a>
...
if ( ! function_exists( 'trav_acc_vacancy_delete_action' ) ) {
	function trav_acc_vacancy_delete_action() {
		...
		// data validation
		if ( empty( $_REQUEST['vacancy_id'] ) ) {
			print __('Maaf, Kamu tidak bisa menghapus apa pun.', 'trav');
			exit;
		}
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			if ( get_current_user_id() != $post_author_id ) {
				print __('Kamu tidak memiliki izin untuk menghapus item lain.', 'trav');
				exit;
			}
		}
...
if ( ! function_exists( 'trav_acc_vacancy_save_action' ) ) {
	function trav_acc_vacancy_save_action() {
		...
		if ( ! isset( $_POST['vacancy_save'] ) || ! wp_verify_nonce( $_POST['vacancy_save'], 'trav_acc_vacancy_manage' ) ) {
		   print __('Maaf, kehadiran kamu tidak diverifikasi oleh sistem.', 'trav');
		   exit;
		} else {

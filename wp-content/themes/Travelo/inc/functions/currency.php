...
if ( ! function_exists( 'trav_currency_converter' ) ) {
    function trav_currency_converter( $amount, $from_currency, $to_currency ) {
        ...
        if ( $converter == 'google_converter' ) {
			...
			$apikey = $trav_options['currency_converter_key'];
			if ( empty( $apikey ) ) {
			...
			if ( ! empty( $trav_options['currency_converter_key_type'] ) && $trav_options['currency_converter_key_type'] == 'premium' ) {
			...
			/*$remote_get_raw = wp_remote_get( "http://free.currencyconverterapi.com/api/v6/convert?q=$from_currency" . "_" . "$to_currency&compact=ultra&apiKey=157eb0b5281c2725fa68" );

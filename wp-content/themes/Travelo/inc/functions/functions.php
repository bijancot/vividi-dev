...
if ( ! function_exists( 'trav_init' ) ) {
    function trav_init() {
        ...
        global $trav_options, $def_currency, $search_max_rooms, $search_max_cabins, $search_max_adults, $search_max_kids, $search_max_passengers, $search_max_flighters, $logo_url, $language_count, $login_url, $signup_url, $redirect_url_on_login, $my_account_page;
        //global variables
...
if ( ! function_exists( 'trav_enqueue_scripts' ) ) {
    function trav_enqueue_scripts() {
        ...
        wp_register_script( 'trav_script_flight', TRAV_TEMPLATE_DIRECTORY_URI . '/js/flight.js', array( 'jquery' ), '', true );
        ...
        wp_localize_script( 'trav_script_main_script', 'week_days', array( 'sunday' => esc_html__( 'sunday', 'trav' ), 'monday' => esc_html__( 'monday', 'trav' ), 'tuesday' => esc_html__( 'tuesday', 'trav' ), 'wednesday' => esc_html__( 'wednesday', 'trav' ), 'thursday' => esc_html__( 'thursday', 'trav' ), 'friday' => esc_html__( 'friday', 'trav' ), 'saturday' => esc_html__( 'saturday', 'trav' ) ) );
        ...
        if ( is_archive('flight' ) ) {
            
            
            wp_enqueue_script( 'trav_script_flight' );
        }
    }
}
...
if ( ! function_exists( 'trav_template_chooser' ) ) {
    function trav_template_chooser($template) {
        ...
        } elseif ( is_search() && $post_type == 'flight' ) {
            return locate_template( 'archive-flight.php' );
        }
        return $template;
    }
}
...
if ( ! function_exists( 'trav_get_available_modules' ) ) {
    function trav_get_available_modules() {
        ...
        if ( empty( $trav_options['disable_cruise'] ) ) $modules[] = 'flight';
        return $modules;
    }
}
...
if ( ! function_exists( 'trav_get_user_booking_list' ) ) {
    function trav_get_user_booking_list( $user_id, $status = 1, $sortby = 'created', $order='desc' ) {
        ...
        if ( in_array( 'flight', $available_modules ) ) {
            $sqls[] = "SELECT 'flight' AS post_type, booking_no, pin_code, total_price, created, status, flight_id AS post_id, arrival_time AS event_date, adults, NULL AS tickets FROM " . TRAV_FLIGHT_BOOKINGS_TABLE . $where;
        }
        ...
        if ( empty( $booking_list ) ) return __( 'Kamu tidak memiliki pesanan perjalanan.', 'trav' ); // if empty return false
        ...
        $cruise_book_conf_url = trav_flight_get_book_conf_url();

        $html = '';
        foreach ( $booking_list as $booking_data ) {
            $class = '';
            $label = 'MENUNGGU';
            if ( $booking_data->status == 0 ) { $class = ' cancelled'; $label = 'BATAL'; }
            if ( $booking_data->status == 2 ) { $class = ' completed'; $label = 'SUKSES'; }
            ...
            if ( 'accommodation' == $booking_data->post_type ) {
				...
				$icon_class = 'soap-icon-car dark-blue-color';
            } elseif ( 'cruise' == $booking_data->post_type ) {
                $conf_url = $cruise_book_conf_url;
                $icon_class = 'soap-icon-cruise orange-color';
            } elseif ( 'flight' == $booking_data->post_type ) {
                $conf_url = $cruise_book_conf_url;
                $icon_class = 'soap-icon-plane orange-color';
            }
            ...
            $html .= '<dl class="info">';
            $html .= '<dt>' . __( 'Tgl. Pesan', 'trav') . '</dt>';
            $html .= '<dd>' . date( 'l, j, M, Y', trav_strtotime( $booking_data->created ) ) . '</dd>';
            $html .= '</dl>';
            $html .= '<dl class="info">';
            $html .= '<dt>' . __( 'KODE BOOK', 'trav' ) . '</dt><dd>' . $booking_data->booking_no . '</dd>';
            $html .= '<dt>' . __( 'PIN', 'trav' ) . '</dt><dd>' . $booking_data->pin_code . '</dd>';
            $html .= '</dl>';
            $html .= '</div>';
        }
        return $html;
